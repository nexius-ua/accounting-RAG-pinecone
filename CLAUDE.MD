...
**Legal RAG system** for Ukrainian intellectual property law using Pinecone vector database for semantic search across legal research documents.

### Domain Focus

- **NDA** (Non-Disclosure Agreement) - Угода про нерозголошення
- **NCA** (Non-Compete Agreement) - Угода про неконкуренцію
- **NSA** (Non-Solicitation Agreement) - Угода про непереманювання
- **ОІВ** (Об'єкти Інтелектуальної Власності) - Intellectual Property Objects
- **Комерційна таємниця** - Trade secrets protection
- **ФОП contracts** - Agreements with individual entrepreneurs
...
```
legal-RAG-pinecone/
├── source_docs/              # Ukrainian legal research documents (.md)
│   ├── Gem 1-17...          # Research gems on specific legal topics
│   ├── Закон про авторське право...  # Relevant legislation
│   └── 13 Expert article... # Professional analyses
├── research/                 # Setup guides and documentation
│   └── Pinecone-Database-Setup-Guide.md
├── scripts/                  # Python scripts for data processing
│   ├── .env                  # Environment variables (API keys)
│   ├── chunk_and_upload.py  # Chunking and Pinecone upload script
│   └── test_chunk_and_upload.py  # Tests for chunking
└── CLAUDE.md                 # This file
```
...
```bash
HOST=https://legal-docs-ua-xxx.svc.pinecone.io  # Pinecone host
PINECONE_API_KEY=pcsk_...                        # Pinecone API key
PINECONE_INDEX=legal-docs-ua                     # Index name
```
...
pip install pinecone python-dotenv pytest

# Run tests
python -m pytest scripts/test_chunk_and_upload.py -v
...
- **KISS**: Choose the simplest solution that works. Avoid clever code
- **Minimal dependencies**: Use vanilla HTML/CSS/JS when possible. Add frameworks ONLY if complexity requires it
- **Low maintenance**: Write code a junior developer could understand in 6 months

### Architecture Rules
- **Flat architecture**: Minimize abstraction layers. Prefer explicit over implicit
- **Standard patterns**: Use well-known, battle-tested solutions over custom implementations
- **Self-documenting**: Clear variable names and simple logic flow over extensive comments
- **Single responsibility**: Each function/module does one thing well
- **Fail gracefully**: Handle errors simply and predictably
- **Progressive enhancement**: Start with a working core, enhance incrementally if needed
...
- **Test coverage**: Aim for comprehensive coverage of core parsing logic
- **Test critical paths**: Focus on document chunking, encoding conversions, edge cases
- **Unit tests for**: Text chunking functions, metadata extraction, ID generation
- **Integration tests for**: Full file parsing, Pinecone uploads, batch processing
- **Test edge cases**: Empty files, corrupted data, large files, Unicode handling
...
- Exotic patterns or technologies
- Deep nesting (callbacks, components, folders)
